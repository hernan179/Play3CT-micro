...
public class Cash5 {
	...
	public static void main(String args[]) throws Exception {
		...
		FileReader fileReader = new FileReader(
				new File(System.getProperty("user.dir") + "/src/main/java/com/example/play3ct/cash5.txt"));
		...
		while ((data = br.readLine()) != null) {
			...
			Integer i = 1;
			if(count == 0) {
				lastNumbers = "====== "+getNum( new Integer(sorter[0]))+" - "+getNum(new Integer(sorter[1]))+" - "+getNum( new Integer(sorter[2]))+" - "+getNum( new Integer(sorter[3]))+" - "+getNum( new Integer(sorter[4]))+" =====";		
			}
		...
		System.out.println("-----Pronostico Cash5 for night para "+new SimpleDateFormat("MM/dd/yyy").format(new Date())+"----------");
		System.out.println(lastNumbers);
		for(int k = 0;k< column1.length;k++) {
			
			String response = "["+getNum(column1[k]) +"] ["+ getNum(column2[k])+"] ["+ getNum(column3[k])+"] ["+ getNum(column4[k])+"] ["+ getNum(column4[k])+"]";
