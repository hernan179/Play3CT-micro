...
public class LuckForLifeAnalyzer {
	...
	public static void main(String args[]) throws Exception {
		...
		FileReader fileReader = new FileReader(
				new File(System.getProperty("user.dir") + "/src/main/java/com/example/play3ct/luckyforlife.txt"));
		...
		while ((data = br.readLine()) != null) {
			...
			//if(count == 0) {
				lastNumbers = "====== "+getNum( new Integer(sorter[0]))+" - "+getNum(new Integer(sorter[1]))+" - "+getNum( new Integer(sorter[2]))+" - "+getNum( new Integer(sorter[3]))+" - "+getNum( new Integer(sorter[4]))+" - "+getNum( new Integer(sorter[5]))+" =====";		
		//	}
		//System.out.println(lastNumbers);
			...
			count++;
			if(new Integer(sorter[4]).equals(47)) {			
			 System.out.println("salio el  hace "+count);
		...
		System.out.println("-----Pronostico lockyforlife para "+new SimpleDateFormat("MM/dd/yyy").format(new Date())+"----------");
		System.out.println(lastNumbers);
		for(int k = 0;k< column1.length;k++) {
			String response = "["+getNum(column1[k]) +"] ["+ getNum(column2[k])+"] ["+ getNum(column3[k])+"] ["+ getNum(column4[k])+"] ["+ getNum(column5[k]) +"] ["+ getNum(column6[k])+"]";
